...
- **DeepSeek is permitted for the demo, on a condition the code cannot check.** The owner first prohibited sending document text outside Malaysia, then reversed it the same day — "反正只是 demo". `build_extractor()` selects `DeepSeekExtractor` when `DEEPSEEK_API_KEY` is set and warns when it does. **The condition is that no real customer document is uploaded while the key is set**, because the risk was never "is this a demo" but "is this document real", and a demo's next move is a prospect's actual questionnaire. Read `AGENTS.md` §3.1 — it records both rulings in order, and the reasoning is the part that matters.
